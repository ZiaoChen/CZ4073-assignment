...
# 3.4.2 Use best size to prune the tree (misclassification)
pTree1 <- prune.misclass(ltree, best = bestSize)
plot(pTree1)
text(pTree1, pretty = FALSE)

# 3.4.3 Find Best Size (deviance)
bestSize <- 1
bestAccuracy <- 0
...
# 3.4.4 Use best size to prune the tree (deviance)
...
# Variable 'Tenure' is the most important variable
importance(rf)
varImpPlot(rf)
...
# Variable 'Sex' is the most important variable
importance(rf)
varImpPlot(rf)
